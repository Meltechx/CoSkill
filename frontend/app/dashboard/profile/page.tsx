...
export default function ProfilePage() {
  ...
  const handleExportCv = () => {
    ...
    const skillRows = summary.skill_scores.length
      ? summary.skill_scores
          .slice()
          .sort((a, b) => b.score - a.score)
          .map((skill) => `
            <li>
              <div class="skill-label"><strong>${escapeHtml(skill.skill)}</strong><span>${skill.score}%</span></div>
              <div class="bar"><i style="width:${Math.min(Math.max(skill.score, 0), 100)}%"></i></div>
              <small>${skill.tasks_count} ${skill.tasks_count === 1 ? "task" : "tasks"} scored</small>
            </li>`)
          .join("")
      : "<p class=\"muted\">No scored skills yet.</p>";

    const completedTasks = summary.recent_completions.length
      ? summary.recent_completions
          .map((task) => `<li><strong>${escapeHtml(task.task_title)}</strong><span>${escapeHtml(task.project_title)} · ${fmtDate(task.completed_at)}</span></li>`)
          .join("")
      : "<p class=\"muted\">No completed tasks recorded yet.</p>";

    printWindow.document.write(`<!doctype html>
      <html><head><title>${escapeHtml(user.full_name || "CoSkill")} — Performance CV</title>
      <style>
        @page { size: A4; margin: 16mm; }
        * { box-sizing: border-box; }
        body { margin: 0; color: #171220; font-family: Inter, Arial, sans-serif; font-size: 11pt; }
        .brand { color: #7c3aed; font-size: 10pt; font-weight: 800; letter-spacing: .15em; text-transform: uppercase; }
        header { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2px solid #ede9fe; padding-bottom: 22px; }
        h1 { font-size: 27pt; margin: 6px 0 4px; letter-spacing: -.04em; }
        h2 { font-size: 13pt; margin: 30px 0 12px; color: #312e81; }
        .subtitle, .muted { color: #6b6675; margin: 0; }
        .score { min-width: 112px; padding: 15px; border-radius: 14px; color: white; text-align: center; background: linear-gradient(135deg, #7c3aed, #2563eb); }
        .score b { display: block; font-size: 28pt; line-height: 1; }.score small { opacity: .8; }
        .stats { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin-top: 24px; }
        .stat { padding: 13px; background: #f7f5ff; border: 1px solid #ede9fe; border-radius: 10px; }.stat b { display: block; font-size: 18pt; color: #4c1d95; }.stat span { color: #6b6675; font-size: 9pt; }
        ul { list-style: none; padding: 0; margin: 0; } .skills li { margin-bottom: 15px; }.skill-label { display: flex; justify-content: space-between; margin-bottom: 6px; }.bar { height: 7px; border-radius: 999px; background: #e9e5f5; overflow: hidden; }.bar i { display: block; height: 100%; border-radius: inherit; background: linear-gradient(90deg, #8b5cf6, #2563eb); }.skills small { color: #77717f; display: block; margin-top: 4px; }
        .tasks li { padding: 10px 0; border-bottom: 1px solid #eeeaf5; }.tasks span { display: block; color: #6b6675; margin-top: 3px; font-size: 9.5pt; }
        footer { margin-top: 32px; padding-top: 12px; border-top: 1px solid #ede9fe; color: #8a8494; font-size: 8.5pt; } @media print { body { print-color-adjust: exact; -webkit-print-color-adjust: exact; } }
      </style></head><body>
        <header><div><div class="brand">CoSkill · Performance CV</div><h1>${escapeHtml(user.full_name || "CoSkill member")}</h1><p class="subtitle">Work performance and skill profile</p></div><div class="score"><b>${summary.overall_score}</b><small>overall score / 100</small></div></header>
        <section class="stats"><div class="stat"><b>${summary.completed_tasks}</b><span>Completed tasks</span></div><div class="stat"><b>${summary.total_tasks}</b><span>Total tasks</span></div><div class="stat"><b>${summary.total_tasks ? Math.round((summary.completed_tasks / summary.total_tasks) * 100) : 0}%</b><span>Completion rate</span></div></section>
        <section><h2>Skill profile</h2><ul class="skills">${skillRows}</ul></section>
        <section><h2>Completed tasks summary</h2><ul class="tasks">${completedTasks}</ul></section>
        <footer>Generated by CoSkill · AI-powered performance tracking · ${new Date().toLocaleDateString()}</footer>
